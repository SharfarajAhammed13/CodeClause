...
def save_file():
    ...
    if file is None:
        file = fd.asksaveasfilename(initialfile='Untitle.txt', defaultextension='.txt', filetypes=[("Text File", "*.txt*"), ("Word Document", '*.docx*'), ("PDF","*.pdf*")])
    else:
        file = open(file, "w")
        file.write(text_area.get(1.0, END))
        file.close()
...
def select_all():
    text_area.event_generate("<<Control-Keyword-A>>")
...
def about_notepad():
   mb.showinfo("About Notepad", "This is just another Notepad, but this is better than all others and its coded by Sharfaraj")
...
file_menu.add_command(label="Close Tab", command=exit_editor)
...
#sources:-https://dev.to/emilossola/building-a-text-editor-gui-with-tkinter-in-python-19l
